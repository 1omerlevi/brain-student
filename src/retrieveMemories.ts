...
export type CandidateTopic = TopicMemory & {
  ...
  last_seen?: string;
  first_seen?: string;
  social_engagement?: { reposts?: number; likes?: number; comments?: number };
  humor_potential?: any;
...
async function fetchCandidatesByMemoryType(
  memoryTypeId: number,
  limit = 200
  ...
  const ids = (mappings ?? []).map((mapping: any) => mapping.topic_id).filter(Boolean);
  if (!ids.length) {
    return [];
  }
  ...
  const { data: topics, error: topicsErr } = await supabase
    .from("topics")
    .select(
      "id, name, summary, category, description, safe_anchors, sensitivity_flags, example_posts, embedding"
    )
    .in("id", ids)
    .limit(limit);
  ...
  const { data: metrics, error: metricsErr } = await supabase
    .from("topic_metrics")
    .select(
      "topic_id, importance_score, recency_score, engagement_score, emotion_metadata"
    )
  ...
  const metricsById: Record<number, any> = {};
...
export async function fetchShortTermCandidates(limit = 200): Promise<CandidateTopic[]> {
  return fetchCandidatesByMemoryType(1, limit);
}

export async function fetchLongTermCandidates(limit = 200): Promise<CandidateTopic[]> {
  return fetchCandidatesByMemoryType(2, limit);
}

export type ScoringContext = {
  now?: Date;
  current_emotion?: string | number;
  current_context_embedding?: number[] | null;
  season_width_days?: number;
  tau_recency_days?: number;
  tau_viral_days?: number;
};
...
export function computeMemoryScore(
  ...
  const daysSince = (timestamp?: string | number | Date) => {
    if (!timestamp) {
      return undefined;
    }
  ...
  if (
    typeof ctx.current_emotion === "number" &&
    ...
    typeof ctx.current_emotion === "string" &&
  ...
  if (Array.isArray(ctx.current_context_embedding) && Array.isArray(memory.embedding)) {
    ...
    const dot = a.reduce(
      (sum: number, value: number, index: number) => sum + value * (b[index] ?? 0),
      0
    );
    const aNorm = Math.sqrt(a.reduce((sum: number, value: number) => sum + value * value, 0));
    const bNorm = Math.sqrt(b.reduce((sum: number, value: number) => sum + value * value, 0));
...
export async function retrieveMemories(
  scene: SceneInput,
  balance = DEFAULT_MEMORY_BALANCE
): Promise<TopicMemory[]> {
  const topK = scene.topK ?? 5;
  const emotion = (scene.reaction_snapshot?.internal_emotion ?? "")
    .toLowerCase()
    .trim();
  const elements = (scene.scene_understanding?.observable_elements ?? []).map((item) =>
    item.toLowerCase()
  );
  ...
  function scoreCandidate(topic: CandidateTopic): CandidateTopic {
    const memoryForScoring: any = {
